// ─────────────────────────────────────────────────────────────────────────────
// CloseOS Sign — coeur des outils MCP (sans SDK ni zod).
//
// Le SDK @modelcontextprotocol/sdk et zod faisaient crasher la fonction Vercel une fois
// bundlés avec supabase/pdf-lib (FUNCTION_INVOCATION_FAILED au chargement). On les retire :
// le protocole MCP (JSON-RPC sur POST) est implémenté à la main dans api/mcp.ts, et ici on
// n'expose que la logique métier (supabase + pdf-lib, tous deux OK au chargement sur Vercel).
//
// 5 outils : importer un contrat EN BROUILLON, poser les champs, vérifier la signature,
// lister, détailler. GARDE-FOU : ne crée que des brouillons (status='draft') et refuse de
// modifier un contrat envoyé/signé/verrouillé. Mono-compte : SIGN_OWNER_EMAIL via service-role.
// ─────────────────────────────────────────────────────────────────────────────
...
// Repère de rendu : pages posées à largeur fixe ; coordonnées = pixels absolus (origine haut-gauche).
...
async function loadPdf(a: { pdf_base64?: string; pdf_url?: string }) {
  let bytes: Buffer
  if (a.pdf_base64) {
...
// ── Handlers des 5 outils ───────────────────────────────────────────────────

async function importContract(args: any) {
  try {
    if (!args?.title) return fail('title requis.')
    const uid = await ownerId()
...
async function placeFields(args: any) {
  try {
    if (!args?.contract_id) return fail('contract_id requis.')
    if (!Array.isArray(args.fields) || args.fields.length === 0) return fail('fields (tableau non vide) requis.')
    ...
    return ok({
      contract_id: contract.id, status: contract.status, mode: args.mode || 'append',
...
async function getStatus(args: any) {
  try {
    if (!args?.contract_id) return fail('contract_id requis.')
...
async function listContracts(args: any) {
  try {
    const uid = await ownerId()
    let q = getClient()
...
async function getContractDetail(args: any) {
  try {
    if (!args?.contract_id) return fail('contract_id requis.')
...
// ── Déclaration des outils (JSON Schema pour tools/list) ─────────────────────
...
export const TOOLS: McpTool[] = [
  {
    ...
    description:
      "Crée un contrat CloseOS Sign EN BROUILLON (status='draft'). Fournir SOIT un PDF (pdf_base64 ou pdf_url), SOIT du contenu texte/HTML (html). Jamais envoyé ni signé. Retourne l'id, les dimensions de page et l'URL d'éditeur.",
    inputSchema: {
      type: 'object',
      properties: {
        ...
        html: { type: 'string', description: 'Contenu HTML du contrat (voie « feuille blanche »). Ignoré si un PDF est fourni.' },
  ...
  },
  {
    ...
    description:
      "Ajoute des champs positionnés (signature, date, texte, checkbox, coordonnées…) à un contrat EN BROUILLON. Coordonnées recommandées en fractions x_pct/y_pct ∈ [0,1] (converties selon les dimensions réelles de la page). REFUSE si le contrat n'est pas 'draft' ou verrouillé.",
    inputSchema: {
      type: 'object',
      properties: {
        ...
        fields: {
          ...
          items: {
            ...
            properties: {
              ...
              w: { type: 'number', description: 'Largeur px (défaut selon type)' },
              h: { type: 'number', description: 'Hauteur px (défaut selon type)' },
              assignee: { type: 'string', enum: ['owner', 'signer'], description: "'signer' (destinataire) ou 'owner' (toi). Défaut signer." },
              signer_index: { type: 'integer', description: 'Numéro du signataire (1..N) si assignee=signer. Défaut 1.' },
              label: { type: 'string' },
              value: { type: 'string', description: 'Valeur pré-remplie (optionnel)' },
            },
            required: ['type'],
          },
        },
        ...
        signers: {
          type: 'array',
          description: 'Renseigne nom/email/téléphone des signataires (aucun envoi).',
          items: {
            type: 'object',
            properties: {
              index: { type: 'integer' },
  ...
  {
    name: 'sign_get_status',
    description: "Statut de signature d'un contrat : statut global (draft/sent/viewed/signed/paid) + détail par signataire. Indique fully_signed.",
    inputSchema: {
      type: 'object',
      properties: { contract_id: { type: 'string', description: 'UUID du contrat' } },
      required: ['contract_id'],
    },
    handler: getStatus,
  },
  {
    name: 'sign_list_contracts',
    description: 'Liste les contrats du propriétaire (id, titre, statut, dates). Filtre optionnel par statut.',
    inputSchema: {
      type: 'object',
      properties: {
        status: { type: 'string', enum: ['draft', 'sent', 'viewed', 'signed', 'paid'], description: 'Filtre optionnel par statut' },
        limit: { type: 'integer', description: 'Nombre max (défaut 25, max 100)' },
      },
    },
    handler: listContracts,
  },
  {
    name: 'sign_get_contract',
    description: 'Détail d’un contrat : signataires, champs déjà posés et dimensions de page (pour positionner de nouveaux champs).',
    inputSchema: {
      type: 'object',
      properties: { contract_id: { type: 'string', description: 'UUID du contrat' } },
      required: ['contract_id'],
