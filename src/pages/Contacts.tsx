...
export function Contacts() {
  ...
  const handleCreateProspect = (prospectData: {
    ...
    // 🔄 HYBRID: Use global context if connected, otherwise show warning
    if (!isConnected || !addProspect) {
      alert('⚠️ Mode Local Déconnecté: Impossible de sauvegarder dans la base globale.\n\nLes données ajoutées seront perdues au rafraîchissement.')
      setIsNewProspectModalOpen(false)
      return
    }

    // Add to GLOBAL context (persists to localStorage automatically)
    addProspect({
      ...prospectData,
