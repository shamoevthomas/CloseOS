...
// --- COMPOSANT TABLEAU SORTI DU COMPOSANT PRINCIPAL POUR EVITER LES ERREURS ---
const MeetingTable = ({ data, title, icon: Icon, emptyText, showDeleteAction, onRefresh, isSyncing, safeFormat, maskData, getMeetingSource, getStatusStyle, getStatusLabel, handleDeleteAllPast, isDeleting, setSelectedMeeting }: any) => (
    <div className="mb-12">
    ...
    <div className="h-full overflow-y-auto bg-slate-950 p-8 text-left">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <MeetingTable 
            ...
            maskData={maskData}
            getMeetingSource={getMeetingSource}
            getStatusStyle={getStatusStyle}
            getStatusLabel={getStatusLabel}
            handleDeleteAllPast={handleDeleteAllPast}
            isDeleting={isDeleting}
            setSelectedMeeting={setSelectedMeeting}
        />
        <MeetingTable data={pastMeetings} title="Historique" icon={History} emptyText="Aucun historique disponible." showDeleteAction={true} safeFormat={safeFormat} maskData={maskData} getMeetingSource={getMeetingSource} getStatusStyle={getStatusStyle} getStatusLabel={getStatusLabel} handleDeleteAllPast={handleDeleteAllPast} isDeleting={isDeleting} setSelectedMeeting={setSelectedMeeting} />
      </div>
      ...
      {isConfigModalOpen && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl flex flex-col animate-in fade-in zoom-in-95 overflow-hidden">
                <div className="flex items-center justify-between p-6 border-b border-slate-800 bg-slate-900 z-10"><div><h2 className="text-xl font-bold text-white flex items-center gap-3">
                  {/* MODIF: Retrait bg-white et p-1 */}
                  <div className="h-8 w-8 flex items-center justify-center overflow-hidden">
                    <img src="/Calcom.png" alt="Cal.com" className="w-full h-full object-contain" />
                  </div>
                  Configuration Cal.com</h2><p className="text-slate-400 text-sm mt-1">Paramètres de connexion et synchronisation.</p></div><button onClick={() => setIsConfigModalOpen(false)} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition-colors"><X className="h-6 w-6" /></button></div>
                             ...
                             {calAccessToken ? (
                                <div className="space-y-4 animate-in fade-in zoom-in-95">
                                 ...
                                 <>
                                     ...
                                     <div className="flex items-center gap-3"><div className="h-px flex-1 bg-slate-800"></div><span className="text-[10px] font-bold text-slate-600 uppercase">OU CLÉ MANUELLE</span><div className="h-px flex-1 bg-slate-800"></div></div>
                                     <div className="flex flex-col gap-2">
                                       ...
                                       {/* AJOUT SUPADEMO JUSTE EN DESSOUS DU BOUTON ENREGISTRER */}
                                       <button 
                                         onClick={() => (window as any).Supademo.open('cmllct87b268o5yi3m4p1uhni')}
                                         className="w-full mt-2 flex justify-center items-center gap-2 rounded-lg bg-slate-800 border border-slate-700 px-4 py-3 text-sm font-bold text-white hover:bg-slate-700 transition-all"
                                       >
