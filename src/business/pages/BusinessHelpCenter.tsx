...
function buildCategories(fr: boolean, hasAcquisition: boolean, _isSolo: boolean): Category[] {
  ...
  cats.push({
    ...
    articles: [
      ...
      { id: 'kpi-relance-rate', title: fr ? 'Taux de réponse aux relances (+ détail par relance)' : 'Follow-up reply rate (+ per follow-up breakdown)', keywords: 'kpi taux de réponse relance détail 1ère 2ème relance sans relance',
        body: [fr ? 'Sur « KPI Setter », la carte « Taux de réponse aux relances » compte les prospects qui ont répondu parmi ceux ayant reçu au moins une relance.' : 'On "Setter KPI", the "Follow-up reply rate" card counts prospects who replied among those who received at least one follow-up.', fr ? 'Le bouton « Détail par relance » ouvre la décomposition : pour chaque rang (1ère, 2ème…), combien de prospects l\'ont reçue et combien ont répondu juste après. Vous voyez ainsi à quelle relance vos prospects répondent vraiment, et si les dernières servent encore.' : 'The "Breakdown per follow-up" button opens the detail: for each rank, how many prospects received it and how many replied right after.'] },
      { id: 'kpi-channel', title: fr ? 'Taux de réponse par canal (écrit / vocal / mail)' : 'Reply rate by channel (written / voice / email)', keywords: 'canal écrit vocal mail message taux de réponse pop-up',
        body: [fr ? 'À chaque premier contact et à chaque « Relance faite », une pop-up demande par quel canal le message est parti : à l\'écrit, par vocal ou par mail.' : 'On every first contact and every "Follow-up done", a pop-up asks which channel the message was sent through.', fr ? 'Ces réponses alimentent le bloc « Taux de réponse par canal » sur « KPI Setter » (canal du premier contact) et la section « Par canal de relance » dans le détail des relances.' : 'These answers feed the "Reply rate by channel" block on "Setter KPI" and the "By follow-up channel" section in the follow-up breakdown.', fr ? 'La case « Ne plus me poser cette question » désactive la pop-up pour vous seul (pas pour l\'équipe). On la réactive dans Paramètres › Interface › « Demander le canal de contact ». Question désactivée = plus de canal enregistré, donc les KPI par canal n\'avancent plus.' : 'The "Stop asking me this" checkbox disables the pop-up for you only. Re-enable it in Settings › Interface.'] },
...
function buildMemberCategories(fr: boolean, role: Role): Category[] {
  ...
  cats.push({
    ...
    articles: [
      { id: 'm-relance', title: fr ? 'Vos relances automatiques' : 'Your automatic follow-ups', keywords: 'relance contacté délai relance faite digest',
        body: [fr ? "Les délais de relance sont définis par votre responsable. Chaque jour, vous recevez la liste de vos relances du jour ; sur la fiche « Contacté », cliquez « Relance faite » pour passer à la relance suivante." : 'Follow-up intervals are set by your manager. Each day you get your due follow-ups; on a "Contacted" card, click "Follow-up done" to move to the next one.', fr ? "À chaque « Relance faite » (et au premier contact), une pop-up vous demande le canal utilisé : à l'écrit, par vocal ou par mail." : 'On each "Follow-up done" (and on first contact), a pop-up asks which channel you used: written, voice or email.'] },
  ...
  if (isS) kpiArticles.push({ id: 'm-kpi-relance-rate', title: fr ? 'Taux de réponse aux relances' : 'Follow-up reply rate', keywords: 'kpi taux de réponse relance détail 1ère 2ème relance',
    body: [fr ? "La carte « Taux de réponse aux relances » compte vos prospects qui ont répondu parmi ceux ayant reçu au moins une relance." : 'The "Follow-up reply rate" card counts prospects who replied among those who received at least one follow-up.', fr ? "Le bouton « Détail par relance » montre, pour chaque rang (1ère, 2ème…), combien l'ont reçue et combien ont répondu juste après — pratique pour voir à quelle relance ça décroche vraiment." : 'The "Breakdown per follow-up" button shows, for each rank, how many received it and how many replied right after.'] })
  if (isS) kpiArticles.push({ id: 'm-kpi-channel', title: fr ? 'Canal de contact & taux de réponse par canal' : 'Contact channel & reply rate by channel', keywords: 'canal écrit vocal mail message pop-up taux de réponse',
    body: [fr ? "À chaque premier contact et à chaque « Relance faite », une pop-up vous demande si le message est parti à l'écrit, par vocal ou par mail." : 'On every first contact and every "Follow-up done", a pop-up asks whether the message was sent in writing, by voice or by email.', fr ? "Vos réponses alimentent le bloc « Taux de réponse par canal » sur vos KPI. Passer la question enregistre quand même le contact, simplement sans canal." : 'Your answers feed the "Reply rate by channel" block in your KPIs. Skipping still records the contact, just without a channel.', fr ? "La case « Ne plus me poser cette question » ne concerne que vous. On la réactive dans Paramètres › Interface › « Demander le canal de contact »." : 'The "Stop asking me this" checkbox only affects you. Re-enable it in Settings › Interface.'] })
