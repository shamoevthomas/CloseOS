// Conditional display rules for questionnaire questions (booking + capture).
// Pure helpers — no React, no fetch. Safe to use server-side too.
...
export function evaluateConditional(
  ...
  // No rule or disabled or invalid (orphan source / type changed) => always show
...
export function buildQuestionsByClientId<Q extends EvaluableQuestion>(
  questions: Q[],
): Record<string, Q> {
  const map: Record<string, Q> = {}
  for (const q of questions) {
    if (q.client_id) map[q.client_id] = q
  }
  return map
}
