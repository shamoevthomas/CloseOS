...
export function DeletionModal({ isOpen, onClose, userEmail, userId, onSuccess }: DeletionModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Suppression du compte</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400">Cette action est importante et irreversible.</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 hover:bg-slate-200 dark:hover:bg-white/5 rounded-lg transition-colors">
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    {step === 'initial' && (
                        <div className="space-y-8 text-center py-8">
                            <div className="p-4 bg-blue-100 dark:bg-blue-500/10 rounded-full w-fit mx-auto">
                                <ShieldAlert className="h-12 w-12 text-blue-600 dark:text-blue-400" />
                            </div>
                            <div>
                                <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Souhaitez-vous supprimer vos données ?</h3>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center mt-8">
                                <button
                                    ...
                                    className="px-6 py-3 bg-red-100 dark:bg-red-500/10 hover:bg-red-200 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 rounded-xl font-bold transition-all border border-red-200 dark:border-red-500/20"
                                >
                                    Oui, supprimer des données
                                </button>
                                <button
                                    onClick={() => handleInitialChoice(false)}
                                    className="px-6 py-3 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-900 dark:text-white rounded-xl font-bold transition-all border border-slate-200 dark:border-white/10"
                                >
                    ...
                    {step === 'select' && (
                        <div className="space-y-6">
                            <div className="p-4 bg-amber-100 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl flex gap-3 text-amber-800 dark:text-amber-200">
                                <AlertTriangle className="h-5 w-5 shrink-0" />
                                <p className="text-sm">Sélectionnez les données que vous souhaitez supprimer définitivement.</p>
                            </div>

                            <div className="space-y-3">
                                <label className="flex items-center gap-3 p-4 bg-slate-100 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 cursor-pointer hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                                    <input
                                        type="checkbox"
                                        checked={scope.includes('all')}
                                        onChange={() => handleScopeChange('all')}
                                        className="w-5 h-5 rounded border-slate-300 dark:border-white/20 bg-white dark:bg-white/10 text-red-600 dark:text-red-500 focus:ring-red-500"
                                    />
                                    <div>
                                        <span className="text-slate-900 dark:text-white font-medium block">Tout Supprimer</span>
                                        <span className="text-slate-500 dark:text-slate-400 text-sm">Compte, facturation, contacts, historiques...</span>
                                    </div>
                                </label>

                                <div className="pl-8 space-y-2">
                                    <label className="flex items-center gap-3 p-3 bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/5 cursor-pointer hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                                        <input
                                            type="checkbox"
                                            checked={scope.includes('all') || scope.includes('personal')}
                                            onChange={() => handleScopeChange('personal')}
                                            disabled={scope.includes('all')}
                                            className="w-4 h-4 rounded border-slate-300 dark:border-white/20 bg-white dark:bg-white/10 text-red-600 dark:text-red-500 focus:ring-red-500"
                                        />
                                        <span className="text-slate-700 dark:text-slate-300">Données personnelles (Nom, email...)</span>
                                    </label>
                                    <label className="flex items-center gap-3 p-3 bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/5 cursor-pointer hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                                        <input
                                            type="checkbox"
                                            ...
                                            className="w-4 h-4 rounded border-slate-300 dark:border-white/20 bg-white dark:bg-white/10 text-red-600 dark:text-red-500 focus:ring-red-500"
                                        />
                                        <span className="text-slate-700 dark:text-slate-300">Données de facturation</span>
                                    </label>
                                    <label className="flex items-center gap-3 p-3 bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/5 cursor-pointer hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                                        <input
                                            type="checkbox"
                                            ...
                                            className="w-4 h-4 rounded border-slate-300 dark:border-white/20 bg-white dark:bg-white/10 text-red-600 dark:text-red-500 focus:ring-red-500"
                                        />
                                        <span className="text-slate-700 dark:text-slate-300">Données de contacts internes</span>
                                    </label>
                                    <label className="flex items-center gap-3 p-3 bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/5 cursor-pointer hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                                        <input
                                            type="checkbox"
                                            ...
                                            className="w-4 h-4 rounded border-slate-300 dark:border-white/20 bg-white dark:bg-white/10 text-red-600 dark:text-red-500 focus:ring-red-500"
                                        />
                                        <span className="text-slate-700 dark:text-slate-300">Données de contacts externes</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                    ...
                    {step === 'confirm' && (
                        <div className="space-y-6 text-center py-8">
                            <AlertTriangle className="h-20 w-20 text-red-600 dark:text-red-500 mx-auto opacity-80" />
                            <div>
                                <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Êtes-vous vraiment sûr ?</h3>
                                ...
                                <div className="max-w-sm mx-auto space-y-4">
                                    <div className="relative text-left">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl pl-11 pr-4 py-3 text-slate-900 dark:text-white focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none transition-all placeholder:text-slate-500 dark:placeholder:text-slate-600"
                ...
                <div className="p-6 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900/50 flex justify-end gap-3">
                    {step === 'select' && (
                        <>
                            <button
                                onClick={onClose}
                                className="px-6 py-2 bg-slate-200 dark:bg-white/5 hover:bg-slate-300 dark:hover:bg-white/10 text-slate-900 dark:text-white rounded-xl transition-colors"
                            >
                    ...
                    {step === 'confirm' && (
                        <>
                            <button onClick={onClose} className="px-6 py-2 bg-slate-200 dark:bg-white/5 hover:bg-slate-300 dark:hover:bg-white/10 text-slate-900 dark:text-white rounded-xl transition-colors">Annuler</button>
                            <button
