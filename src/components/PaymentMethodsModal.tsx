        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formType === 'VIREMENT' && (
                  <div className="space-y-3 p-4 bg-[#111111] rounded-2xl border border-white/[0.08]">
                    <input type="text" placeholder="Banque" value={formBankName} onChange={e => setFormBankName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-2 text-white placeholder:text-white/30 focus:border-emerald-500 focus:outline-none" />
                    ...
                    <input type="text" placeholder="Titulaire" value={formAccountHolder} onChange={e => setFormAccountHolder(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-2 text-white placeholder:text-white/30 focus:border-emerald-500 focus:outline-none" required />
                  </div>
                )}
                ...
                {formType === 'STRIPE' && <input type="text" placeholder="Lien Stripe" value={formPaymentLink} onChange={e => setFormPaymentLink(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-2 text-white placeholder:text-white/30 focus:border-emerald-500 focus:outline-none" required />}
          ...
          {methods.length === 0 && !isAdding ? (
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-12 text-center">
              ...
              <p className="text-white/40">Aucun moyen de paiement.</p>
            </div>
          ) : (
                ...
                <div key={method.id} className="group relative rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4 transition-all hover:bg-white/10">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => confirm('Supprimer ?') && deleteMethod(method.id)} className="p-2 text-white/40 hover:text-red-400"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </div>
                </div>
              ))}
