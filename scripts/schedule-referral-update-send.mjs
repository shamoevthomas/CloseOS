// Programmation Brevo de l'email "Les règles de parrainage ont changé"
// Envoi à 4 destinataires avec leur cycle/code/nom personnalisés.
// Usage: BREVO_API_KEY=xxx node scripts/schedule-referral-update-send.mjs

const BREVO_API_KEY = process.env.BREVO_API_KEY;
if (!BREVO_API_KEY) { console.error('❌ BREVO_API_KEY manquant'); process.exit(1); }

// Cible : 7 mai 2026 à 14:30 Europe/Paris (CEST = UTC+2)
const SCHEDULED_AT = '2026-05-07T14:30:00+02:00';

const DESTINATAIRES = [
    { email: 'personnatcindy@gmail.com',         name: 'Cindy',   cycle: 'yearly',  code: 'N23RA2M' },
    { email: '123adelinevasseur.pro@gmail.com',  name: 'Adeline', cycle: 'monthly', code: 'PVBZGPX' },
    { email: 'jorge.reisviegas@gmail.com',       name: 'Jorge',   cycle: 'monthly', code: 'QS2Z25N' },
    { email: 'tekatubois@gmail.com',             name: 'Thomas',  cycle: 'monthly', code: 'SUQYJAC' },
];
...
async function scheduleEmail(to, name, subject, htmlContent, scheduledAt) {
...
const SUBJECT = 'Les règles de parrainage ont changé 🔄';

console.log(`📅 Programmation Brevo pour ${SCHEDULED_AT} (heure Paris)`);
console.log(`👥 ${DESTINATAIRES.length} destinataires :\n`);

const results = [];
for (const d of DESTINATAIRES) {
    const html = referralUpdateEmail(d.name, d.code, d.cycle);
    const r = await scheduleEmail(d.email, d.name, SUBJECT, html, SCHEDULED_AT);
    let parsed = null;
    try { parsed = JSON.parse(r.body); } catch {}
    const messageId = parsed?.messageId || parsed?.messageIds?.[0] || '—';
    console.log(`  ${r.ok ? '✅' : '❌'} ${d.email.padEnd(40)} | ${d.cycle.padEnd(10)} | ${r.ok ? messageId : 'FAIL ' + r.status + ' ' + r.body}`);
    results.push({ ...d, ok: r.ok, messageId, body: r.body });
}

console.log('\n📋 Récap pour annulation/contrôle (DELETE https://api.brevo.com/v3/smtp/email/{messageId}) :');
for (const r of results) {
    if (r.ok) console.log(`  - ${r.email}: ${r.messageId}`);
}
